...
verifyLine(Board, WorkerRow, WorkerColumn, Row, Column,Index, Res, 'S'):-
      (Row =:= WorkerRow + Index, Column =:= WorkerColumn, Res is 1, write('Line 28 RES: '), write(Res), nl); %S
      (WorkerRowTemp is WorkerRow + Index, getValueFromMatrix(Board, WorkerRowTemp, WorkerColumn, Value), Value \= empty, Res is 0,  write('Line 29 '), write(Value), write(' RES: '),write(Res), nl);
      (write('Line 30\n'), Index < 12,
      Index1 is Index + 1,
      verifyLine(Board, WorkerRow, WorkerColumn, Row, Column, Index1, Res, 'S'), write('Line 32\n')).
